...
update_ewea <- function(user , pass , date_to_update){
  ...
  # 
  # ewea_raw%>%filter(Member_name=="KAALO" & District_Name=="Jariiban")%>%select(dataID,reporting_month ,Reporting_date,submission_time, Member_name ,District_Name, Community_name ,Residence_type )%>%
  # arrange(Residence_type,Member_name)%>%view()
  # 
  ewea_raw%<>%filter(
  ...
  ewea_raw%<>%
    mutate(
      District_Name = case_when(
        Residence_type=="Rural" & District_Name == "Galkacyo" & Member_name == "IRC" ~ "Galkacyo-South",
        Residence_type=="Rural" & District_Name == "Galkacyo" & Member_name == "KAALO" ~ "Galkacyo-North",
        .default = District_Name
        ),
      District_Name = case_when(
        Residence_type=="Urban" & District_Name == "Galkacyo" & Member_name == "KAALO" ~ "Galkacyo-North",
        .default = District_Name
      )
    )
  # 
  # ewea_raw%>%
  #   filter(Member_name == "IRC" & District_Name=="Galkacyo-South")%>%select(reporting_month,submission_time,Residence_type)
  # 
  # nrow(ewea_raw)
  # 
  # ewea_raw%>%select(submission_time,reporting_month,Member_name,District_Name)
  
  # ewea_raw%<>%
  #   mutate(
  #     Member_name = str_replace(Member_name , "_"," ")
  #   )%>%
  #   separate_rows(Member_name,sep = " ")%>%
  #   filter(is.na(Project_name) | Project_name == 'BRCIS3')
    
  
  # ewea_raw%>%arrange(submission_time)%>%
  #   group_by(reporting_month,Member_name,District_Name) %>%
  #   summarise(
  #     communities = paste(Community_name,collapse = ","),
  #     Residence_type = paste(Residence_type,collapse = ","),
  #     Project_name = paste(Project_name,collapse = ","),
  #     number_of_submission = n()
  #   )%>%filter(number_of_submission>1)%>%view()
  #   
  # write.xlsx(ewea_raw,"new early warning data.xlsx")
  
  ewea_raw%<>%arrange(desc(submission_time))%>%
    group_by(reporting_month,District_Name,Residence_type) %>%
    slice(1) %>%
    ungroup() %>%
    as.data.frame()
  ...
  rainfall <- ewea_raw%>%
    select(reporting_month,Member_name,District_Name,Residence_type,Rainfall_category,temp,Cumulative_rain)%>%
  ...
  flash_floods <- ewea_raw%>%
    select(reporting_month,Member_name,District_Name,Residence_type,Flash_flood,Flash_flood_days,Flashfloods_aff_hhs)%>%
    mutate(
  ...
  river_levels <-ewea_raw%>%
    select(reporting_month,Member_name,District_Name,River_levels,
  ...
  awd_cases <-ewea_raw%>%
    select(reporting_month,Member_name,District_Name,awd_cases,awd_cases_previous_week, awd_cases_treated,
  ...
  locust <- ewea_raw%>%
    dplyr::select(reporting_month,Member_name,District_Name,locust_infest,spent_days,  locust_effect,Residence_type)%>%
    mutate(reporting_month = as.yearmon(reporting_month)) %>%
    ## change the values to labels using custom function
  ...
  conflict <- ewea_raw%>%
    dplyr::select(reporting_month,Member_name,District_Name,conflict_presence,
